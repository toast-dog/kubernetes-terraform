...
locals {
  vip_patch     = templatefile("${path.module}/patches/vip.yaml", { talos_vip = var.talos_vip })
  install_patch = templatefile("${path.module}/patches/install-disk.yaml", { talos_image = var.talos_image })
}
...
data "talos_machine_configuration" "controlplane" {
  ...
  config_patches     = [local.vip_patch, local.install_patch]
}
...
resource "talos_machine" "controlplane" {
  for_each = toset(local.controlplane_ips)
